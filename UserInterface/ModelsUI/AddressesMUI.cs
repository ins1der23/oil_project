        ...
        while (flag)
        {
            ...
            switch (choice)
            {
                ...
                case 3: // Изменить номер дома и квартиру
                    await ShowString(item.ShortString(), clear: true);
                    string houseNum = await GetStringAsync(RegistrationText.changeHouseNum, clear:false);
                    string flatNum = await GetStringAsync(RegistrationText.changeFlatNum, clear: false);

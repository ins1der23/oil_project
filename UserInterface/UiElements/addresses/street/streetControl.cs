    ...
    public class StreetControl
    {
        public static async Task<Street> Start(City city)
        {
            ...
            while (mainFlag)
            {
                ...
                if (street.Id == 0)
                {
                    choice = await MenuToChoice(StreetText.searchAgainOrAddStreet, invite: CommonText.choice, noNull: true);
                    switch (choice)
                    {
                ...
                else
                {
                    ...
                    while (levelOneFlag)
                    {
                        choice = await MenuToChoice(StreetText.options, street.Name, invite: CommonText.choice, noNull: true);
                        switch (choice)
                        {
                            case 1: // Выбрать
                                await ShowString(StreetText.streetChoosen);

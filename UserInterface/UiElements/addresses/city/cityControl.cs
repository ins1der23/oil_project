    ...
    public class CityControl
    {
        public static async Task<City> Start()
        {
            ...
            while (mainFlag)
            {
                ...
                if (city.Id == 0)
                {
                    choice = await MenuToChoice(CityText.searchAgainOrAddCity, invite: CommonText.choice, noNull: true);
                    switch (choice)
                    {
                ...
                else
                {
                    ...
                    while (levelOneFlag)
                    {
                        choice = await MenuToChoice(CityText.options, city.Name, invite: CommonText.choice, noNull: true);
                        switch (choice)
                        {
                            case 1: // Выбрать
                                await ShowString(CityText.cityChoosen);

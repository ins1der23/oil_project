...
namespace Handbooks
{
    public class RegistrationControl
    {
        public static async Task<Registration> Start()
        {
            ...
            while (mainFlag)
            {
                choice = await MenuToChoice(RegistrationText.addOrChange, registration.LongString, invite: CommonText.choice, noNull: true);
                switch (choice)
                {
                    ...
                    case 2: // Искать среди существующих
                        ...
                        if (registration.Id != 0)
                        {
                            while (levOneFlag)
                            {
                                choice = await MenuToChoice(RegistrationText.options, registration.LongString, invite: CommonText.choice, noNull: true);
                                switch (choice)
                                {
                                    case 1: // Выбрать 
                        ...
                        else
                        {
                            ...
                            choice = await MenuToChoice(CommonText.yesOrNo, AddrText.addAddress, invite: CommonText.choice, noNull: true);

    ...
    public class ChangeRegistration
    {
        public static async Task<Registration> Start(Registration registration, bool toSql = true)
        {
            ...
            while (flag)
            {
                ...
                choice = await MenuToChoice(RegistrationText.changeOptions, string.Empty, Text.choice, clear: false);
                switch (choice)
                {
            ...
            if (registration.SearchString != registrationOld.SearchString)
            {
                ...
                if (choice == 1)
                {
                    ...
                    else
                    {
                        ...
                        if (toSql) registration = await registrations.SaveChanges(user, registration);
                        return registration;
                    }
                }
            }

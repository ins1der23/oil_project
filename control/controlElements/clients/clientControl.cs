...

{
    public class ClientControl
    {
        public static async Task Start()
        {
            ...
            while (mainFlag)
            {
                int choice = await MenuToChoice(ClientText.findClient, ClientText.menuName, Text.choice); // Меню поиска
                switch (choice)
                {
                    case 1:  // Найти клиента
                        ...
                        while (levelOneFlag)
                        {
                            ...
                            choice = await MenuToChoice(clientList.ToStringList(), ClientText.clientFound, Text.choiceOrEmpty);
                            if (choice != 0)
                            {
                            ...
                            {
                                choice = await MenuToChoice(ClientText.searchAgainOrAddClient, invite: Text.choice);
                                switch (choice)
                                {
                        ...
                        if (clientToChange.AddressId != 0)
                        {
                            ...
                            while (levelOneFlag)
                            {
                                await ShowString(clientToChange.Summary(), delay: 100);
                                choice = await MenuToChoice(ClientText.options, invite: Text.choice, clear: false, noNull: true); // Меню выбора клиента
                                switch (choice)
                                {
